				...
				<div class="row" style="overflow: scroll;">
					<div class="col-md-8">
						...
						<div class="box box-info">
							...
							<!-- /.box-header -->
							<!-- form start -->
							<form:form action="editPayment" method="post"
								...
								<div class="box-body">
									<div class="form-group">
										...
										<div class="col-sm-4">
											<input type="date" name="date" id="date" value="${salesPaymenet.date}">
										</div>
										...
										<select required id="customerId" name="customerId">
												<c:forEach items="${customerList}" var="customerList">
													<c:choose>
													    <c:when test="${salesPaymenet.customerId == customerList.id}">
													    	<option selected value="${customerList.id}">${customerList.name}</option>
													    </c:when>    
													    <c:otherwise>
									...
									<div class="form-group">
										...
										<div class="col-sm-4">
											...
											<c:set value="${sessionScope.MKEYVAL['ADDPAY']}" var="mIdKeyValMap"></c:set>
												...
												<c:set var="key" value="${mKeyValMap.key}"></c:set>
													<c:choose>
													    <c:when test="${key==salesPaymenet.type}">
										...
										<div class="col-sm-4">
											<select id="mode" name="mode">
												...
												<c:forEach items="${mIdKeyValMap}" var="mKeyValMap">
													<c:choose>
													    <c:when test="${mKeyValMap.key==salesPaymenet.mode}">
									...
									<div class="form-group">
										...
										<div class="col-sm-10">
											<input type="text" class="form-control" id="comment" name="comment"
												value="${salesPaymenet.comment}" placeholder="Comment" value="">
										</div>
									</div>
								</div>
								<input type="hidden" id="id" name="id" value="${salesPaymenet.id}"/>
								<!-- /.box-body -->
								<div class="box-footer" align="center">
									<button type="reset" class="btn btn-default">Reset</button>
									<button type="submit" class="btn btn-info">Edit Payment</button>
								</div>
								<!-- /.box-footer -->
							</form:form>
						</div>
