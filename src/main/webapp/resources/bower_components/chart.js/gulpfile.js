var gulp = require('gulp'),
	...
	fs = require('fs'),
	package1 = require('./package1.json'),
...
gulp.task('build', function(){
	...
	return gulp.src(srcFiles)
		...
		.pipe(replace('{{ version }}', package1.version))
 ...
 *	Output: - New version number written into package1.json & bower.json
 */
...
gulp.task('bump', function(complete){
	...
	inquirer.prompt({
		...
		// Set the new versions into the bower/package1 object
		package1.version = newVersion;
		bower.version = newVersion;

		// Write these to their own files, then build the output
		fs.writeFileSync('package1.json', JSON.stringify(package1, null, 2));
